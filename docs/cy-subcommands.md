...
方式を取る（`resolveReadWindow`）。窓は描画済みスクロールバックに対する
range/limit 操作であり、「過去のその瞬間の画面」ではなく **最終状態の行**
を返す点に注意。優先順位（先勝ち）:

1. `--range A:B` — 1 始まり包含の明示窓
2. `--before-line L [--limit N]` — 行 L の **直上** で終わる N 行のページ
3. `--head N` / `--last N` — 先頭 / 末尾 N 行
