...
- **`needs_input` is a query-time *derived* state, not a persisted enum.**
  ...
  `ay status`/`ay ls` *already render the agent's screen* (for the activity and
  ...
  - *Rejected:* writing `needs_input` into `pids.jsonl` from the runtime. It
...
- **Detection is runtime-agnostic — so it is also dual-runtime "parity" for
  free.** The signal is the *drawn menu*, which looks identical whether the agent
...
- **Reused `--watch` as the event stream instead of a new callback channel.**
  ...
  - *Rejected:* a notify-file / unix-socket callback. It is redundant with the
...
- **The detection seam is "a selection menu the auto-responder declined."**
  ...
  prompts. What is *left* on a stable screen is exactly the genuine question
...
- **Backward compatibility was the top constraint.** No existing state value or
  flag changed meaning. `needs_input` only *renames a case previously conflated
  with idle*. `--wait-idle` is byte-for-byte unchanged (its help now points at
...
- **Global watch stream — `ay ls --watch --json`.** A single event stream of
  state transitions across *all* agents (`{pid, event, question?, cwd}`), so a
  fan-out parent watches one process instead of N per-pid `--watch`es. *Why not
  now:* the per-pid `--watch`/`--wait` already unblocks the reported scenario;
  the global stream is the natural next increment (diff successive `ay ls`
  snapshots and emit transitions).
- **[P3] `--on-question` mode.** Let a spawn opt into auto-answering or
  ...
  parent over a channel). *Why not now:* needs a policy model + a safe default;
...
- **[P4] Structured result envelope + `ay result <pid>`.** Have the agent emit a
  ...
  instead of the parent grepping `ay tail`. *Why not now:* requires a convention
...
- **[P5] `ay wait --until=<state,...>`.** Generalize `--wait` to arbitrary
  target-state sets and multiple pids at once. *Why not now:* `--wait` covers the
...
- **[P6] Stall heartbeat.** Detect an agent that is neither working nor at a
  ...
  `stalled` state. *Why not now:* needs a separate liveness signal beyond mtime;
...
- **[P7] `--lock` advisory.** An advisory per-cwd / per-submodule lock so two
  ...
  hand-bundles them). *Why not now:* there is already a `runningLock`; extending
...
- **Native runtime detection.** The runtime (which already pattern-matches the
  ...
  *Why not now:* query-time detection already works and is runtime-agnostic; a
